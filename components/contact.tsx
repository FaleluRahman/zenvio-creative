...
export default function Contact() {
  ...
  const WHATSAPP_NUMBER = "918714974849";

  const getWhatsAppUrl = () => {
    const selectedServicesText = selected.length > 0 ? selected.join(", ") : "General Inquiry";
    const formattedText = `Hello Zenvio Creative!\n\n*Services Needed:* ${selectedServicesText}\n\n*Message:* ${message.trim()}`;
    return `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(formattedText)}`;
  };
        ...
        >
          ...
          {sent ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 px-8 text-center">
              ...
              <p className="text-[15px] mb-6 max-w-md" style={{ color: textMuted, fontFamily: "system-ui, -apple-system, sans-serif" }}>
                Your message has been formatted. If WhatsApp didn&apos;t open automatically, click the button below to start your conversation.
              </p>
              <a
                href={getWhatsAppUrl()}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 font-semibold text-white px-7 py-3.5 rounded-xl transition-all duration-300"
                style={{
